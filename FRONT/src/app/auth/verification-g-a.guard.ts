...
export class Verification_G_A implements CanActivate {
  constructor(private http: HttpClient, private router: Router) {}

  canActivate(route: any): Observable<boolean> | Promise<boolean> | boolean {
      return this.http.get<{ valid: boolean }>('http://localhost:3000/verification_G_ou_A', { withCredentials: true }).pipe(
      map((response) => {
        if (response.valid) {
