...
module.exports = (db) => {
    ...
    router.post('/depot', upload.single('image'), async (req, res) => {
        ...
        console.log(email_vendeur, nom_jeu, prix_unit, quantite_deposee, est_en_vente, editeur, description, num_session);

        const emailConnecte = req.session.email_connecte;
        let x = 0;

        if (est_en_vente === "true") {
            x = 1;
            console.log("vraie");
        }
        ...
        try {
            ...
            if (req.file) {
                ...
                imagePath = `/IMAGE/${req.file.filename}`; // Chemin du fichier enregistré
            }

            // Chemin par défaut si aucune image n'est fournie
            if (!imagePath) {
                imagePath = '/IMAGE/pas_photo.JPG';
            }
            ...
            res.status(200).send({ message: 'Jeu ajouté avec succès', imagePath });
        } catch (error) {
